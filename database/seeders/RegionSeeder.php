...
class RegionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $provinces = Http::get("https://wilayah.id/api/provinces.json")->json(
            "data"
        );
        ...
        foreach ($provinces as $province) {
            ...
            $this->command->info(
                "Fetching Regencies for {$province["name"]}...."
            );
            ...
            foreach ($regencies as $regency) {
                ...
                $this->command->info(
                    "Fetching Districts for {$regency["name"]}...."
                );

                $districts = Http::get(
                    "https://wilayah.id/api/districts/{$regency["code"]}.json"
                )->json("data");
                ...
                foreach ($districts as $district) {
                    ...
                    $this->command->info(
                        "Fetching Villages for {$district["name"]}...."
                    );
